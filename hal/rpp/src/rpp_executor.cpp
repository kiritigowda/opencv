...
int runRppReduce(const RppBuf* srcs, int nSrc,
    ...
    if (nSrc < 0 || nSrc > kMaxSrc || !resultOut || resultBytes == 0)
    ...
    if (path == RPP_GPU) {
        ...
        if (ok) {
            devResult = allocHipBuffer(resultBytes);
            ok = (devResult != nullptr);
        }

        rppHandle_t handle = nullptr;
        if (ok) {
            handle = createRppGpuHandle(1);
            ok = (handle != nullptr);
        }
        if (ok) {
            ok = op(devSrc, nSrc, devResult, handle, backend);
            clearStickyHipError();
        }
        if (ok) {
            ok = (hipMemcpy(resultOut, devResult, resultBytes, hipMemcpyDeviceToHost) == hipSuccess);
        }

        for (int i = 0; i < nSrc; ++i) freeHipPtr(devSrc[i], srcBytes[i]);
        freeHipPtr(devResult, resultBytes);
        return ok ? CV_HAL_ERROR_OK : CV_HAL_ERROR_NOT_IMPLEMENTED;
    }
#endif

    // CPU (HOST) path: RPP writes the result array directly into host memory.
    rppHandle_t handle = createRppCpuHandle(1);
    if (!handle) return CV_HAL_ERROR_NOT_IMPLEMENTED;
    ...
    bool ok = op(hostSrc, nSrc, resultOut, handle, backend);
    destroyRppCpuHandle(handle);
    return ok ? CV_HAL_ERROR_OK : CV_HAL_ERROR_NOT_IMPLEMENTED;
}
